...
This makes the Cosmos Hub the ideal zone to deploy Peggy. The high value of staked ATOM allows for enormous amounts of value to be stored in the Peggy Ethereum contract without breaking this rule. Small zones can still of course run Peggy, but their lower staking token value will result in a lower cap on the total funds that can be locked in the bridge.
